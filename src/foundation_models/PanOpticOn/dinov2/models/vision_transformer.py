...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        if embed_layer == 'PatchEmbed':
            assert len(pe_args) == 0
            self.patch_embed = PatchEmbed(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
            num_patches = self.patch_embed.num_patches
        elif embed_layer == 'ChnAttnPatchEmb':
            defaults = dict(
                patch_emb_cfg = dict(
            ...
            def _rec_update(old:dict, new:dict):
                for k,v in new.items():
                    if k not in old or not isinstance(old[k], dict):
                        old[k] = v
                    else:
                        _rec_update(old[k],v)
                return old
            pe_args = _rec_update(defaults, pe_args)

            self.patch_embed = ChnAttnPatchEmb(**pe_args)
