...
class DinoV2Adapter(nn.Module):
    def __init__(self, config, pretrain_size=224, num_heads=12, conv_inplane=64, n_points=4, deform_num_heads=6,
...
class DinoV2AdapterSegmentation(LightningTask):
    def __init__(self, args, config, data_config):
        super().__init__(args, config, data_config)
        self.adapter_backbone = DinoV2Adapter(config, interaction_indexes=[[0, 2], [3, 5], [6, 8], [9, 11]])
        if config.freeze_backbone:
    ...
    def params_to_optimize(self):
        backbone_params_list = [param for param in self.adapter_backbone.parameters() if param.requires_grad]
        return backbone_params_list + list(self.decoder.parameters()) + list(self.aux_head.parameters())
