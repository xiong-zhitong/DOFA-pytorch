    ...
    for model_folder in os.listdir(logs_dir):
        ...
        for exp in experiments:
            ...
            if not runs.empty:
                ...
                # Add hyperparameters
                collect_hyper_params = ["param_params.args/lr", "param_params.args/batch_size", "param_params.args/model", "param_params.args/dataset"]
                run_info.update({
                    f'param_{k}': v.strip() for k, v in best_run.items() if k in collect_hyper_params
                })
...
def save_best_metrics(logs_dir, output_file='best_lightning_metrics.csv', 
