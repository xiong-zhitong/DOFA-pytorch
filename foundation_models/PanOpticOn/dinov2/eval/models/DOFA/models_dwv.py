...
class OFAViT(nn.Module):
    ...
    def forward_features_nopool(self, x, waves):
        # embed patches
        # waves = torch.tensor(wave_list, device=x.device, requires_grad=False).float()
        waves = waves.float()
        waves.requires_grad = False
    ...
    def forward_features(self, x, wave_list):
        x = self.forward_features_nopool(x, wave_list)
