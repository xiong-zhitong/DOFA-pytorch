...
class UperNet(torch.nn.Module):
    ...
    def forward(self, x):  
        outputs = self.backbone.get_intermediate_layers(x, self.idx_blocks_to_return)
...
class SoftCON(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = dinov2_vit.__dict__[config.softcon_size](
            img_size=config.image_resolution, patch_size=14,
            in_chans=config.num_channels, block_chunks=0,
            init_values=1e-5,
