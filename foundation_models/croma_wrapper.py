...
class CROMA(nn.Module):
    def __init__(self, config):
        super(CROMA, self).__init__()

        self.encoder = PretrainedCROMA(pretrained_path=config.pretrained_path, \
                size=config.size, modality=config.modality, image_resolution=config.image_resolution)
        ...
        if config.task == 'classification':
            #add linear layer
            self.encoder.GAP_FFN_s2[1] = torch.nn.Linear(self.encoder.GAP_FFN_s2[1].in_features, config.num_classes)
            del self.encoder.GAP_FFN_s2[2:]
            self.model = self.encoder

        elif config.task == 'segmentation':
            #del self.encoder.GAP_FFN_s2
            self.freeze(self.encoder.GAP_FFN_s2)
