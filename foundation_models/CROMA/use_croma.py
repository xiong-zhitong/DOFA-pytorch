...
class PretrainedCROMA(nn.Module):
    ...
    def forward(self, SAR_images=None, optical_images=None):
        ...
        if self.modality in ['optical', 'both']:
            ...
            optical_encodings, out_feats = self.s2_encoder(imgs=optical_images, attn_bias=self.attn_bias.to(optical_images.device))  # (bsz, num_patches, encoder_dim)
...
class BaseTransformer(nn.Module):
    ...
    def forward(self, x, relative_position_bias=False):
        ...
        for self_attn, ffn in self.layers:
            ...
            k = int(math.sqrt(K))
            if i in out_indices:
                out_features.append(x.reshape(B,k,k,C).permute(0,3,1,2).contiguous())
            i += 1
