...
class SegGeoBenchTransform(object):
    ...
    def __call__(self, sample):
        array, band_names = sample.pack_to_3d(band_names=self.band_names, resample=True, fill_value=None, resample_order=3) # h,w,c
        array = torch.from_numpy(array.astype('float32')).permute(2,0,1)
...
class GeoBenchDataset():
    ...
    def create_dataset(self):
        ...
        assert task is not None, f"{self.dataset_config.dataset_name} doesn't exist in geobench"
        GeoBenchTransform = ClsGeoBenchTransform if self.dataset_config.task == 'classification' else SegGeoBenchTransform

        train_transform = GeoBenchTransform(task, split='train', size=self.img_size, band_names=self.dataset_config.band_names)
        val_transform = GeoBenchTransform(task, split='valid', size=self.img_size, band_names=self.dataset_config.band_names)

        dataset_train = task.get_dataset(split="train", transform=train_transform, band_names=self.dataset_config.band_names)
        dataset_val = task.get_dataset(split="valid", transform=val_transform, band_names=self.dataset_config.band_names)
        dataset_test = task.get_dataset(split="test", transform=val_transform, band_names=self.dataset_config.band_names)
